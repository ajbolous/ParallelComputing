...
class Kernel
{
  ...
  public:
    ...
    {
        int ret;
        printf("\n# Allocating memory buffer\t  -size: %d\t  -Ptr: %d\n", size, data);
        if (flags == NULL)
    ...
    void execute(char *name, int dim, size_t *globalWorkSize, size_t *localWorkSize)
    {
        ...
        printf("\n# Executing Kernel %s\t -dims: %d", name, dim);
