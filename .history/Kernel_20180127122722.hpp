...
class Kernel
{
  ...
  public:
    ...
    void setArg(int index, int size, void *data)
    {
        ...
        if (data == NULL)
        {
            printf("Setting LocalArg: index:%d, size:%d,  Ptr: %d", index, size, data);
        ...
        printf("Setting arg: index:%d, size:%d,  Ptr: %d", index, size, data);
    ...
    void getResult(int argIndex, int size, void *result)
    {
        ...
        if (ret != CL_SUCCESS)
            printf("Error reading mem buffer CODE: %D", ret);
    }
