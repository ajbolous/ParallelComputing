...
class Kernel
{
	...
	std::unordered_map<int, cl_mem> _memory;
  ...
  public:
	...
	{
		int ret;
		_memory[index] = clCreateBuffer(_context, CL_MEM_READ_WRITE, size, data, &ret);

		if (ret != CL_SUCCESS)
			printf("Error allocating memory CODE: %d\n", ret);

		clSetKernelArg(_kernel, index, sizeof(cl_mem), (void *)&_memory[index]);
	}
	...
	{
		clEnqueueReadBuffer(_queue, _memory[index], CL_TRUE, 0, size, result, 0, NULL, NULL);
	}
...
class OpenClLoader
{
  ...
  public:
	...
	Kernel *load_kernel(char *fileName, char *kernelName)
	{
		...
		Kernel k = Kernel(context, commandQueue, program, kernel);
