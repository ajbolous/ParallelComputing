...
{
	int *eqmap = (int *)malloc(sizeof(int) * width * height);
	int *imageMap = (int *)malloc(sizeof(int) * width * height);
		...
		for (int j = 0; j < width; j++)
		{
			...
			if (image[index] >= 1)
			{
				...
				if (upn == 0 && ln == 0)
				{
					...
					if (counter < 0)
						counter = 0;
					eqmap[counter] = counter;
					image[index] = (uchar)counter;
				}
				else if (upn == 0)
					image[index] = ln;
				else if (ln == 0)
