...
int main()
{
    ...
    fkernel->setBufferArg(1, WIDTH * HEIGHT * sizeof(int), labels);
    ...
    fkernel->getResult(1, sizeof(int) * WIDTH * HEIGHT, labels);

    Kernel *kernel = ocl.load_kernel("pccl.cl", "twoStagePccl");
    kernel->setBufferArg(0, WIDTH * HEIGHT * sizeof(int), labels);
    ...
    kernel->getResult(0, sizeof(int) * WIDTH * HEIGHT, labels);

    for(int i =0;i<height;i++)
    for(int j =0;j<width, j++)
        image.data[i*width + j] = (uchar)labels[i*width+j];
