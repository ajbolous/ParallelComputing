...
int main()
{
    ...
    int* imageLabel = (int*)malloc(sizeof(int) * width  * height);
    
    fkernel->setBufferArg(0, WIDTH * HEIGHT * sizeof(int), image.data);
    ...
    fkernel->getResult(0, sizeof(int) * WIDTH * HEIGHT, image.data);

    printf("Starting next stage \n\n");
    Kernel *kernel = ocl.load_kernel("pccl.cl", "twoStagePccl", 0);
    kernel->setBufferArg(0, WIDTH * HEIGHT * sizeof(int), image.data);
    ...
    kernel->getResult(0, sizeof(int) * WIDTH * HEIGHT, image.data);
