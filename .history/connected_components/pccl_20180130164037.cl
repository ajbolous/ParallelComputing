__kernel void twoStagePccl(__global int* image,  __global int *changed, int width, int height)
{
...
__kernel void firstRunPccl(__global int *image, int width, int height)
{
    int row = get_global_id(1);
    int col = get_global_id(0);
    int index = row * width + col;
    if ((row < height) && (col < width) && image[index] > 0){
        image[index] = index/8000 + 1;
    }
}