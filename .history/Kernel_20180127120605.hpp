...
class Kernel
{
  ...
  public:
    ...
    void setArg(int index, int size, void *data)
    {
        if (data == NULL)
        {
            clSetKernelArg(_kernel, index, size, NULL);

            return;
        }
        int ret;
        _mem[index] = clCreateBuffer(_context, CL_MEM_READ_WRITE, size, NULL, &ret);
        clEnqueueWriteBuffer(_queue, _mem[index], CL_TRUE, 0, size, data, NULL,NULL,NULL);
    ...
    {
        clEnqueueReadBuffer(_queue, _mem[argIndex], CL_TRUE, 0, size, result, 0, NULL, NULL);
    }
