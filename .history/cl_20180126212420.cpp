...
class OpenClLoader
{
  ...
  public:
	...
	Kernel *load_kernel(char *filename)
	{
		...
		if (ret != CL_SUCCESS)
			printf("Error Creating program from source CODE:%d", ret);
		...
		if (ret != CL_SUCCESS)
			printf("Error Building program from source CODE:%d", ret);

		/* Create OpenCL Kernel */
		cl_kernel kernel = clCreateKernel(program, filename, &ret);

		if (ret != CL_SUCCESS)
			printf("Error Creating kernel from source CODE:%d", ret);
