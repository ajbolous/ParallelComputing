...
class OpenClLoader
{
  ...
  public:
	...
	int set_kernel_args(cl_kernel kernel, void *args)
	{ /* Set OpenCL Kernel Parameters */

		clSetKernelArg(kernel, 0, sizeof(cl_mem), (void *)&allocatedMemroy);
	}
	...
	int invoke_kernel(char* filename){
		...
		set_kernel_args(kernel, NULL);
		execute_kernel(kernel);
	}

	invoke_kernel_with_args(char* filename){
