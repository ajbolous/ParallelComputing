...
#define STG2WG (NUMWORKGROUPS / WGSIZE + 1)
int main()
{
	OpenClLoader ocl;
	...
	kernel->setArg(2, sizeof(int), (void *)&vecSize);
	kernel->setArg(3, NUMWORKGROUPS * sizeof(double), NULL);
	...
	int resultSize = NUMWORKGROUPS;
	kernel->setArg(0, NUMWORKGROUPS * sizeof(double), result);
	kernel->setArg(1, STG2WG * sizeof(double), result2);
	kernel->setArg(2, sizeof(int), (void *)&resultSize);
	kernel->setArg(3, STG2WG * sizeof(double), NULL);
