...
class Kernel
{
  ...
  public:
    ...
    void execute(char *name, size_t globalWorkSize[], size_t localWorkSize[], void *result, int resArg, int argCount, ...)
    {
        ...
        for (int i = 0; i < argCount; ++i)
        {
            ...
            setArg(i, sizeof(arg), arg);
        }
        va_end(args);

        if (sizeof(localWorkSize) > 1 && localWorkSize[0] != 0)
            globalWorkSize[0] = ((globalWorkSize[0] - 1) / localWorkSize[0] + 1) * localWorkSize[0];
        if (sizeof(localWorkSize) > 1 && localWorkSize[1] != 0)
        ...
        getResult(resArg, sizeof(result), result);
    }
