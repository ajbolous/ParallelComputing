...
void initValues (int nWorkItems, double *input, int nWorkGroups, double *output) {
   ...
   // Init values for input array
   for (i=0; i<nWorkItems; i++)
      input[i] = i+1;

   // Init values for output array
...
int main (int argc, char** argv) {
   ...
   if (argc != 3) {
     fprintf(stderr,"Bad number of Arguments : Size of Array , Size of WorkGroup\n");
     // Exit with error
     exit(1);
   }
   else {
     size = atoi(argv[1]);
     sizeWorkGroup = atoi(argv[2]);
   }
   ...
   if (clEnqueueReadBuffer(command_queue, reductionBuffer, CL_TRUE, 0, nWorkGroups*sizeof(double), 
